import { X, Info, Check, CircleAlert, Mail, Dot, UserRoundPen } from 'lucide-react';
import { useState } from 'react';
import api from '../../services/api';
import PersonalInfoSection from '../admin/PersonalInfoSection';
...
function buildFormFromUser(user?: User | null): RegisterUserFormState {
  return {
    nombre: user?.nombre || '',
...
export default function EditUserModal({ isOpen, onClose, user, onSaveSuccess }: EditUserModalProps) {
  const [form, setForm] = useState<RegisterUserFormState>(() => buildFormFromUser(user));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cleanForm: RegisterUserFormState = {
      ...form,
    ...
    try {
      ...
      if (status === 409) {
        ...
        setGeneralError('No tienes permisos para editar usuarios.');
      } else if (status === 401) {
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <h2 id="edit-user-title" className="text-sm font-bold text-[#011140] sm:text-xl">
                  Editar Usuario
                </h2>
                <p className="mt-0.5 text-[11px] leading-snug text-gray-500 sm:mt-1 sm:text-xs">
                  <span className="sm:hidden">Modifica los datos del miembro</span>
            ...
            <div className="mt-4 hidden sm:block">
              <div className="flex items-start gap-3 rounded-lg border border-[#DBEAFE] bg-[#EFF6FF] p-3">
                ...
                <p className="text-[0.70rem] leading-relaxed text-[#011140]">
                  Toda acción de edición quedará auditada bajo la norma de seguridad académica institucional.
                </p>
              </div>
            </div>
